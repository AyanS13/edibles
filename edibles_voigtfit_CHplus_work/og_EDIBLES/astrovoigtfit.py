...
def astro_voigt_fit(
    ...
    # Finding   maximum number of components across all species......
    ...
    # groups of b similar to v_rad...
    b_groups = {}
    for comp_idx in range(max_components):
        b_groups[comp_idx] = {}
        for species_idx in species_params:
            if comp_idx < species_params[species_idx]['b'].size:
                b_value = species_params[species_idx]['b'][comp_idx]
                if b_value not in b_groups[comp_idx]:
                    b_groups[comp_idx][b_value] = []
                b_groups[comp_idx][b_value].append((species_idx, comp_idx))

    # === Create master parameters for b values ===
    master_b_params = {}

    def make_valid_param_name_b(comp_idx, b_value):
        value_str = str(b_value).replace('-', 'neg').replace('.', 'p')
        return f'master_b_{comp_idx}_{value_str}'

    for comp_idx in b_groups:
        for b_value in b_groups[comp_idx]:
            param_name = make_valid_param_name_b(comp_idx, b_value)
            params.add(param_name, value=b_value, min=0.5, max=5.5, vary=True)
            master_b_params[(comp_idx, b_value)] = param_name
    ...
    for species_idx in species_params:
        ...
        for i in range(n_component):
            b_value = species_data['b'][i]
            master_b_name = master_b_params[(i, b_value)]
            params.add(f'b_{species_idx}_{i}', expr=master_b_name)

            params.add(f'N_{species_idx}_{i}', value=species_data['N'][i], min=0)

            v_rad_value = species_data['v_rad'][i]
            master_v_name = master_v_rad_params[(i, v_rad_value)]
            params.add(f'v_rad_{species_idx}_{i}', expr=master_v_name)
