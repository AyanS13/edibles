#################################################################
# fitting without continuum 
#################################################################
from edibles.utils.final_voigt_profile import *

# from edibles.utils.britannia_voigt_profile  import *   #importing the ultimate Voigt profile model with map 
...
def multi_voigt_absorption_line( **params_list):
    ...
    model = voigt_absorption_line_new(wavegrid, lambda0=all_lambda, f=all_f, gamma=all_gamma, b=all_b,B_vary= B_vary, N=all_N, v_rad=all_v_rad, 
...
def fit_multi_voigt_absorptionlines(wavegrid=np.array, ydata=np.array, restwave=np.array, f=np.array, gamma=np.array, 
             b=np.array,B_vary = None, N=np.array, v_rad=np.array, v_resolution=0., n_step=0, std_dev = 1,map =np.array):
    """
...
#----------------Now create the Parameters class----------------------------------------------
    ...
    if n_trans == 1:
        params.add('lambda0', value=restwave[0], vary=False)
        params.add('f0', value=f[0], vary=False)
        params.add('gamma0', value=gamma[0], vary=False)
    ...
    params.add('B_vary', value=B_vary[0], vary=False)
...
if __name__ == "__main__":
    ...
    if show_example == 1:
        ...
        List = pythia.getFilteredObsList(object=["HD 149757"], MergedOnly=False, Wave=6708.0)
        ...
        # map =[0,0,1,1]
        restwave = [6707.757,6707.769,6707.908,6707.920,6707.922,6707.925,6708.073,6708.076]
        f = [0.1855,0.30910,0.09270,0.15460,0.16490,0.32970,0.08243,0.16490]
        gamma = [3.69e7, 3.69e7, 3.69e7, 3.69e7,3.69e7, 3.69e7, 3.69e7, 3.69e7]
        map = [0,0,0,0,1,1,1,1]
        # print(wave)
        ...
        fitresult = fit_multi_voigt_absorptionlines(wavegrid=wave, ydata= flux,f= f, restwave=restwave, gamma=gamma, 
                            b=[2],B_vary = [0], N=[7.113e+09,7.113e+09/3.5], v_rad=-11, v_resolution=3, n_step=25, map =map,std_dev= 0.0035554091884622933)
        fitresult.params.pretty_print()

        print("chi-square value ",fitresult.chisqr)
        print("reduced chi-square value ",fitresult.redchi)
        # Li6_7_ratio =  r"\frac{Li^6}{Li^7}" + "=" + str(1/(fitresult.params['R'].value))
        # display(Math(Li6_7_ratio))
        print("ratio:",fitresult.params['N0'].value/fitresult.params['N1'].value)

        plt.plot(wave, flux,color ='gray',label ='data')
        plt.plot(wave,fitresult.best_fit,color ='purple',label ="fit")
        plt.xlabel("Wavelength ($\AA$)")
        plt.ylabel("Normalised flux")
        plt.title("Single cloud model fit for Li",color = 'darkgreen')
        plt.grid()
        plt.xlim(6707,6708.2)
        plt.legend()
        plt.show()
        ...
        #############################################################
        #
        # EXAMPLE 2: Na doublet: multiple lines, single cloud.
        #
        #############################################################
        ...
        fitresult = fit_multi_voigt_absorptionlines(wavegrid=wave, ydata= flux,f= f, restwave=lambda0, gamma=gamma, 
                               b=b,B_vary=[0], N=N, v_rad=v_rad, v_resolution=v_resolution, n_step=25, map =[0,0],std_dev= 0.0035554091884622933)
        fitresult.params.pretty_print()
        ...
        plt.xlabel("Wavelength ($\AA$)")
        plt.ylabel("Normalised flux")
        plt.title("Single cloud model fit for Na",color = 'darkgreen')
        plt.grid()
        plt.legend()
        plt.show()
        ...
        #############################################################
        #
        # EXAMPLE 3: 12CH+ and 13CH+ lines: multiple lines, single cloud.
        #
        #############################################################
        ...
        fitresult = fit_multi_voigt_absorptionlines(wavegrid=wave, ydata= flux,f= f, restwave=lambda0, gamma=gamma, 
                               b=b, B_vary=B_vary, N=N, v_rad=v_rad, v_resolution=v_resolution, n_step=25, map =[0,1],std_dev= 0.0015)
        fitresult.params.pretty_print()
        ...
        plt.xlabel("Wavelength ($\AA$)")
        plt.ylabel("Normalised flux")
        plt.title("Single cloud model fit for CH+",color = 'darkgreen')
        plt.grid()
        plt.legend()
        plt.show()
        ...
        fitresult = fit_multi_voigt_absorptionlines(wavegrid=wave, ydata= flux,f= f, restwave=lambda0, gamma=gamma, 
                               b=b,B_vary=B_vary, N=N, v_rad=v_rad, v_resolution=v_resolution, n_step=25, map =map ,std_dev= std_dev)
        fitresult.params.pretty_print()
        ...
        plt.xlabel("Wavelength ($\AA$)")
        plt.ylabel("Normalised flux")
        plt.title("Single cloud model fit for CN",color = 'darkgreen')
        plt.grid()
        plt.legend()
        plt.show()
